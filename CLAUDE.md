...
NPMPublishPlugin is a Publish plugin that runs npm build steps as part of the static-site publishing pipeline.
...
Depends on sibling `Publish` (and remote swift-subprocess). Monorepo uses a relative path; standalone CI rewrites Publish via `Scripts/ensure-remote-deps.sh`.
